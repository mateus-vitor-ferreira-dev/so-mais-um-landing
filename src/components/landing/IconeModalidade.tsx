...
/**
 * O desenho de cada modalidade, casado pelo `icon` que a api serve.
 *
 * Até a web#511 nove das doze eram emoji — o `iconFallback` da api — e só
 * futevôlei, vôlei de areia e peteca tinham SVG próprio, porque o Unicode não
 * tem desenho para elas (#46). A #46 deixou em aberto o caminho **C**, trocar
 * as doze por um conjunto único, e era o único que resolvia o problema de
 * fundo: **emoji não é desenho, é fonte**. A fileira de modalidades tinha uma
 * cara no iPhone, outra no Android e outra no Windows, e três cartões
 * denunciavam a origem ao lado dos nove da fonte de quem visita.
 *
 * O `iconFallback` continua no `lib/sports.ts` e o `contrato:check` continua
 * conferindo: é o contrato com a api, e o app ainda o usa. Ele só deixou de ser
 * o que a landing desenha.
 *
 * As regras são as da #46, agora para as doze:
 *
 * 1. **Volume.** Gradiente com a luz vindo de cima à esquerda e, nas bolas, a
 *    camada `esfera` por cima escurecendo a borda oposta.
 * 2. **Enquadramento apertado.** O `viewBox` encosta no desenho, para o ícone
 *    ocupar a caixa inteira e não parecer menor que o vizinho.
 * 3. **A silhueta é intocável.** Nada de cor escura na borda, e toda linha
 *    interna acompanha a curvatura — é o que faz uma bola de 16px ler como
 *    bola.
 *
 * E uma regra nova, que só aparece com as doze juntas: **duas bolas não podem
 * ter a mesma cara**. Vôlei é branco com costura cinza, porque o azul e amarelo
 * já é do vôlei de areia; tênis é verde-limão, porque o amarelo já é do
 * futevôlei; futsal é a chuteira, porque a bola já é do society.
 *
 * Os `id` de gradiente saem do `useId`: o mesmo ícone aparece no mockup do app
 * e na grade de modalidades, e dois `<linearGradient id="x">` na mesma página
 * fazem o segundo desenho herdar o primeiro — ou sumir, se o primeiro estiver
 * dentro de um bloco escondido.
 */

type Id = (nome: string) => string
...
/** Os `icon` da api que têm desenho próprio. */
export const ICONES_DESENHADOS = Object.keys(DESENHOS)

export default function IconeModalidade({
  icon,
  tamanho = 30,
  className,
}: {
  /** O `icon` que a api serve em `GET /sports` (`beach-tennis`, `volei`…). */
  icon: string
  tamanho?: number
  className?: string
}) {
  // O `useId` do React traz caracteres que não valem dentro de `url(#…)`.
  const base = `modalidade-${useId().replace(/[^a-zA-Z0-9_-]/g, '')}`
  const desenho = DESENHOS[icon] ?? GENERICO
